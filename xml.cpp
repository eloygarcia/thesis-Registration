...
size_t XML :: Char2Base64(char *Z, int s, char *out)
	{
		int L = s;
		char *oo;
		Base64Encoder<char*,char*>e;
...
int XML :: Base642Char(char *Z, int s, char *out)
	{
		if (s == -1)
		s = (int)strlen(Z);
		int L = s;
		char *oo;
		Base64Decoder<char*,char*>e;
		oo = e.Filter(out, Z, Z + L);
		oo = e.Finish(oo);
		*oo = 0;                     // Put a zero to the end
		return (oo - out);
...
#else
						strncat_s(vval,jj - j + 1000,txt + k,1);
#endif						
						}
					cv->SetValue(vval);
...
#else
			strncat_s(LastData,10000,txt + j,1);
#endif
			continue;
			}
		}
...
static HCRYPTPROV CryptAck()
	...
	if (!p && GetLastError() == NTE_KEYSET_NOT_DEF)
		{
		a = MS_ENH_RSA_AES_PROV_XP;
		CryptAcquireContext(&p,0,a,PROV_RSA_AES,CRYPT_VERIFYCONTEXT);
		}

	if (!p && GetLastError() == NTE_BAD_KEYSET)
